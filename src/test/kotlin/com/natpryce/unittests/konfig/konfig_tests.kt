...
class FromEnvironment {
    ...
    @Test
    fun `translates camelCase property names to proper env var names`() {
        val env = mapOf("LOWER_CAMEL_CASE" to "a", "UPPER_CAMEL_CASE" to "b")
        val config = EnvironmentVariables(lookup = { env[it] })

        val a = Key("lowerCamelCase", stringType)
        val b = Key("UpperCamelCase", stringType)

        assertThat(config[a], equalTo("a"))
        assertThat(config[b], equalTo("b"))
    }

    @Test
    fun `translates special chars to proper env var names`() {
        val env = mapOf("ENV___VAR___NAME" to "a")
        val config = EnvironmentVariables(lookup = { env[it] })

        val a = Key("env|,|var@$@name", stringType)

        assertThat(config[a], equalTo("a"))
    }

    @Test
    fun `does not convert proper env var names any further`() {
        val env = mapOf("ENV_VAR_NAME" to "a")
        val config = EnvironmentVariables(lookup = { env[it] })

        val a = Key("ENV_VAR_NAME", stringType)
