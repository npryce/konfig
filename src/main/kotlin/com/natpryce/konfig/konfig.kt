...
class ConfigurationProperties(
    ...
    companion object {
        ...
        fun fromFile(file: File) =
            load(if (file.exists()) file.inputStream() else null, Location(file.absolutePath, file.toURI())) {
...
class EnvironmentVariables(
    ...
    private fun toEnvironmentVariable(name: String): String {
        require(name.isNotEmpty()) {
            "Cannot convert empty name to environment variable name."
        }

        return buildString {
            append(prefix)

            name.fold(false) { separatorAllowed, c ->
                when (c) {
                    in 'A'..'Z' -> {
                        if (separatorAllowed) append("_$c")
                        else append(c)
                        false
                    }
                    in 'a'..'z' -> {
